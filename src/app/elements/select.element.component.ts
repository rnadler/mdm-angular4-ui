...
@Component({
  ...
  template: `<div [attr.id]="path" *ngIf="defaultItem" [hidden]="hidden">{{context?.label}}<br>
    <select [attr.id]="path + '.select'" (change)="onChange($event.target.value)" [(ngModel)]="defaultItem">
